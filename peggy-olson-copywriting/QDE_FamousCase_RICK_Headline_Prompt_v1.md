# QDE Famous-Case — RICK Headline Prompt
**Version:** v1 · **Prepared:** 2026-09-15 · **Stage A of** `QDE_FamousCase_Story_Prompt_V2.md` · **Lane:** QDE only
...
Column J (image prompt) and column L (caption) are written afterwards by Peggy/Claude — the image prompt is built
...
Full acceptance test is in `QDE_FamousCase_Story_Prompt_V2.md`. The short version — all six must pass:
...
5. Every proper noun, date and number traces to column N. **RICK invents confidently** — anything it added on its
   own goes back through the Research Prompt before use.
...
LINCOLN, which is a word the headline does not contain. Use the block above, not the live row.

⚠️ Four other live rows have the same defect — **FC-8, FC-10, FC-14 and FC-15 have headlines that no longer contain
their gold word**, and **FC-2 (`HOWARD HUGHES`) and FC-15 (`SECRET DIARY`) have two-word gold words.** The graphic
gilds the gold word *from the headline*, so where the word is absent there is nothing to gild. Worth a pass.
