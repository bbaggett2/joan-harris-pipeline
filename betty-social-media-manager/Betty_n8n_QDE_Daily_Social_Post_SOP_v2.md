...
**Version:** v2 · **Prepared:** 2026-09-12 · v2: LinkedIn confirmed as Bart's personal profile; Metricool userId filled in · **Lane:** Handwriting Experts Inc. / QDE only
...
**Files:** `Betty_n8n_QDE_Daily_Social_Post_v2.json` (import into n8n) · `QDE_Post_Queue_seed_v1.csv` (first five rows for the Google Sheet)
...
## Google Sheet — `QDE_Post_Queue`, tab `Queue`
...
| :--- | :--- | :--- |
| Google Sheets — Baggett | Google OAuth2 | the account that owns the queue sheet |
...
Then fill the **Config** node: sheet id, Drive folder id, reviewer Slack channel. `blogId` `6268508` and `userId` `4831552` are already set (read from the live Metricool account 2026-09-12).
