# HU Brand Thumbnail v3 — The Screenshot + Torn Parchment Method
...
**Default path:** the Path A creation process — `HU_Thumbnail_PathA_Generated_Host_v<n>.md`,
generated host. Use that unless told otherwise.
**Last updated:** 2026-09-12
**Rewritten:** 2026-08-14

> **Versioning:** the version is in the filename and the highest number wins. A new revision
> becomes `..._v4_...md` and this file is deleted in the same commit. Cross-references never name
> an exact version — `Something_v<n>.md` means the highest-numbered one in that folder.
>
> **What changed in v3:** the two approval gates in §5 are gone, replaced by the pipeline's single
> review. And every reference that pointed at `HU_Thumbnail_Creation_Process_v5.md` now names the
> Path A creation process without a version — v5 was retired and replaced, so this file had been
> routing readers to a dead document.
...
| Governing file | the Path A creation process | This file, plus Path A for everything shared |
...
This document is an **override**, not a second SOP. It replaces exactly one section of the Path A
creation process and inherits the rest.

| Topic | Where it lives |
| :--- | :--- |
| **Host image sourcing** | **This file, §1–§5. Replaces Path A §6 entirely.** |
| Deliverable order, canvas sizes, platform specs | Path A §0.5 — unchanged |
| Colour palette | Path A §2 — unchanged |
| Lettering, fonts, emboss | Path A §3 — unchanged |
| The torn edge — fractal noise, octaves, corner taper | Path A §4 — unchanged |
| Parchment sizing, the three layouts, where words go | Path A §5 — unchanged |
| The HU logo, placement, safe zones | Path A §7 — unchanged |
| Trait handwriting — published library, never generated | brand kit §6 item 2 — unchanged, **applies on both paths** |
| Build checklist | Path A §8, with steps 2–3 and 9 swapped for §5 below |

**Nothing above is restated here.** Duplicating craft rules across two files is how they drift
apart. If you need the tear algorithm, open the Path A creation process §4 — or
`HU_Thumbnail_Constants_v<n>.md`, which is the numbers. Do not work from memory of it.
...
- Bart simply wants the real thing for this one.
...
> On Path A, generate at the target aspect ratio instead — the Path A creation process §0.5 Step 2
> forbids padding a landscape plate into a vertical canvas.

---

## 5. Build steps that differ from the Path A build checklist

Run the Path A creation process §8 as written, with these substitutions:

**Step 1 — YouTube 16:9**

- *Path A steps 2–3 (generate plate, recognition check)* → **replaced by:** grab the frame from
  the actual video per §2 above; reject bad frames early.
- Everything else in Path A §8 Step 1 is unchanged, including the trait card, the composite order,
  and the 2 MB ceiling.

**Step 2 — Vertical 9:16**

- *Path A step 9 (regenerate plate at 9:16)* → **replaced by:** grab a 9:16-suitable frame, or pad
  a landscape frame per §4.
- *Path A step 10* onward is unchanged — layout by title length, type sizing, tear, composite,
  asserts, `ig_v1.jpg`.

**No approval gate on either deliverable.** Salvatore self-checks and hands off. The thumbnails
are seen once, by the reviewer, on the private YouTube video and in the Metricool drafts — making
the video public and scheduling the drafts is the approval. A rejected thumbnail re-runs its step
alone and swaps on the live video: a redo, not a failure.

*v2 held at "Bart's approval gate" on the 16:9 and "Bart approves" on the 9:16. Both were removed
when the pipeline replaced four per-phase sign-offs with one review.*

Filenames are identical on both paths: `yt_v1.jpg`, `ig_v1.jpg`, incrementing, **never a status
word**. A file is named for which attempt it is, not for what anyone thinks of it.
...
*HU Brand Thumbnail — The Screenshot + Torn Parchment Method* (v4) §6, which the Path A process
retired. Nothing in them is new. §0, §1, and §5 are routing and ordering, drafted by an agent on
2026-08-14 to implement Bart's ruling that the screengrab method be preserved as an opt-in
alternate rather than deleted.

### 6.2 This partially un-retires a Path A retirement

The Path A retirement table lists "Pull the frame from the actual video" as retired, and says the
screengrab method is retired *entirely*. That is now too strong. **That table needs one line
amended** to read that the screengrab method is demoted to an opt-in alternate path governed by
this file — not deleted. Until that edit is made, the two files disagree.

Two Path A retirements are **unaffected** and still stand on both paths:
...
Path B material, not dead code as the Path A open items assumed. **That item needs amending** —
the frame-grab and headliner-padding logic must be *preserved and gated*, not stripped.

---

## 7. Open items

1. **Amend the Path A retirement table** so it describes the screengrab as demoted, not deleted.
   See §6.2.
2. **Amend the Path A open item on dead code.** Frame-grab, hairline detection, and headliner
   padding in `build_hu_ig_thumbnail.py` are live Path B code. Gate them behind a path flag; do
   not delete.
...
6. **Verify the Path A filename.** This file now routes to the Path A creation process without a
   version. The repo currently holds `HU_Thumbnail_PathA_Generated_Host_v6.md`; confirm that is
   the intended name before relying on it, since older documents still call it
   `HU_Thumbnail_Creation_Process_v5.md`.
