...
>
> **What changed in v2:** §11 only — the approver attribution in the naming rule. Every number,
> geometry value, measurement and design ruling is identical to v1.
...
*v1 said "only Bart approves, so only Bart names anything approved." The approver is now a role
rather than a person, and the naming rule no longer depends on who it is.*
