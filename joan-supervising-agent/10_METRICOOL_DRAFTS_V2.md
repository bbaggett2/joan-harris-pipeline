...
All four drafts now take `long_url`, the Drive download URL that `Media facts` builds. That matches the standing rule that QDE and HU videos publish in their native aspect with no vertical re-cut — see file 08, which documents a vertical-cut lane that was never built.
