...
This document does not include the n8n workflow. This process will be rewritten asap.
>
...
vertical cut**. No duration test decides either one. Reels and the feed are separate surfaces
with separate audiences; publishing one leaves the other empty.

V13's Phase 8 table already created four drafts with two of them Facebook, but the prose
underneath still said "still splits on 90 seconds" and contradicted its own table. That is
fixed, and so is the consequence V13 missed: **the two Facebook posts need two different
media files**, so Phase 8 Step 1 now stages both the full-length and the vertical cut.

⚠️ **The real Facebook constraint is FILE SIZE, not duration** — a hard **500 MB** cap through
Metricool. Now that the POST always carries the full-length master, this is what will actually
break a job. Check byte size before creating it.

**2. Brand is resolved once, at pickup, and carried.** *(Bart, 2026-09-14.)* The
Ready-to-Publish folder the video came out of determines the brand. That value is written to
the job and travels with it to Metricool, where it selects the `blogId`. **Nothing downstream
re-infers the brand.**
...
| *(QDE's folder — see the QDE pipeline's OPEN FLAGS; not yet identified)* | QDE |
...
1. **`PipelineState` row for the VID_ID**, in the master spreadsheet — job state belongs with
   job state.
2. **`<VID_ID>_Description_Package_v1.md`** — the one file Betty reads. She takes the lane
   from the package and is forbidden from inferring it from the video, folder, or title.

**Everything brand-dependent hangs off that one value** — the Metricool `blogId`, the YouTube
channel, the Facebook page ID, the description prompt, the thumbnail method, CTA policy and
banned terms.

⛔ **An unresolvable brand is an error, not a guess.** No folder match, or content that
straddles two brands → STOP and report. Never pick.
...
Metricool blogId     : 6267975                  ← selected by the resolved brand value
...
Ready-to-Publish     : Drive folder 1yRdzkR3wGdvDDPuR2NeLRrIC6pXLKQTv
                       ...
                       ⭐ THIS FOLDER IS THE HUMAN APPROVAL **and** the brand signal.
...
exists — they are different deliverables. **Both are used**: the long cut carries YouTube and
the Facebook POST, the short vertical cut carries TikTok, Instagram and the Facebook REEL.
...
| **Metricool** | All five posts are drafts. A draft is not scheduled and cannot fire. | The reviewer schedules them. |
...
- **A brand that will not resolve**, or content that belongs to two brands
...
- **Gate 1 — Brand.** Resolve the brand from the Ready-to-Publish folder ID the video came
  ...
  is HU. If it resolves to QDE or Bart Show, use that brand's pipeline file. If it will not
  resolve, or the content straddles two brands, STOP.
...
Peggy delivers **five** copy outputs in one pass — YouTube, Instagram, TikTok, Facebook
short, Facebook long — plus the title from Phase 2.
...
4. **Record the byte size of the full-length cut.** *(New in V14.)* It is not a routing
   decision — it is the one thing that can block the Facebook POST at Phase 8. Over 500 MB
   means compress before that post can be created; see the compression recipe.
...
2. Upload to the channel named by the **resolved brand** — for HU, `UCG-DLule9ZSStBdc5gSEoCw`
   (`@handwritinguniversity`).
...
⛔ Read the **resolved brand** from the package. It selects the `blogId` — HU is `6267975`.
Never type a `blogId` from memory and never infer the brand here.
...
(⚠️ OPEN FLAGS). Nothing is staged, copied, or deleted.
...
**Fallback — the FTP bridge.** Use this *only* when a Drive URL fails with
...
| Draft | Providers | networkData | Media | Copy (verbatim from the package) |
|---|---|---|---|---|
...
duplicate content. **No duration test decides either type:** the POST always takes the
full-length cut, the REEL always takes the short vertical cut. Precedent: HU VID-GD04,
2026-08-18 — full-length POST plus a 0:33 vertical REEL.
...
⚠️ **A failed `createScheduledPost` leaves nothing behind.** There is no partial draft to
find in the planner. If a whole network set fails at once, suspect the media file — a
landscape master breaks Instagram (aspect), Facebook (500 MB) and TikTok simultaneously.
...
bio", the right `blogId` for the resolved brand, and `media` now a `static.metricool.com` URL.
...
- [ ] All four on the `blogId` for the resolved brand
...
- [ ] `media` is a `static.metricool.com` URL on every draft
...
6. **Bart Show / BAB** — `brands/bartallanbaggett-brand.md` exists, but no description
   prompt, no pipeline, and no Ready-to-Publish folder do. It cannot be brand-resolved.
