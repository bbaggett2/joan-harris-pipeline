...
Facebook page ID     : 348215195822          ⚠️ UNVERIFIED — see Open Flags
...
1. **Facebook page ID `348215195822`** — confirm it routes to the HU brand page, not a
   personal page. One-time check, then delete this flag.
2. **QDE pipeline not forked.** V7's QDE content still has no home. Create
   `QDE_Video_Publishing_Pipeline_V9.md`. ⛔ Keep V7 in git history until it exists.
3. **Bart Show / BAB** — `brands/bartallanbaggett-brand.md` exists, but no description
...
5. **`BART_BAGGETT_VOICE.MD` is committed three times** — `brands/`, `betty/`, `peggy/`,
   all identical SHA. Keep `brands/`. Delete the other two.
