# 11 — HU Preflight
**V2 · 2026-09-19 (evening) · Governs n8n node: `Preflight` (Code) · Replaces `11_PREFLIGHT_V1.md`**
...
> **Two assertions were removed in V2** because they contradicted other files in this folder. Both are recorded at the bottom so nobody re-adds them.
...
| No destination we do not own — another platform named as somewhere to go | `OFF_PLATFORM_DESTINATION` | flag |
| The four captions are distinct from each other | `CAPTIONS_NOT_DISTINCT` | flag |
| Hashtag counts match the per-platform numbers in the prompt; `#graphology` only as a hashtag | `HASHTAG_COUNT` | flag |
...
⚠️ **`CAPTIONS_NOT_DISTINCT` has never actually fired in anger.** No run in either lane has yet been checked for four genuinely different captions. Treat the first HU run as the test.
...
⚠️ **`FORMAT_MISMATCH` is only as good as `yt_format`.** File 02's boundary rule changed in V2 because a 180.499-second video was classed `short` and YouTube disagreed by half a second. A file within a second or two of three minutes may be classed either way; this assertion will happily confirm the copy matches a classification that is itself wrong.

## Removed in V2 — do not re-add

**`MODE_NOT_ENUM`.** File 05 removed `mode` from the HU contract on 2026-09-19: it was copied from the QDE spec, where it selects attorney-facing versus consumer-facing framing, and HU has one audience. The assertion could only ever fire on a field this contract does not have.

**The "hashtags 5–12" count.** File 06 removed that number for the same reason it is wrong here: V3 sets counts **per platform** — YouTube 12–18, Instagram 8–12, TikTok 4–6, Facebook REEL none needed. A flat 5–12 contradicts the prompt for YouTube and would flag correct copy. The prompt wins; point at it rather than copying numbers into this folder.
