# 07 — HU Thumbnails
**V2 · 2026-09-19 (evening) · Governs n8n nodes: `Thumbnail: job` · `Write thumbnail job` · `Wait for thumbnail worker` · `Parse thumbnail result` · and the HU thumbnail worker on the Mac · Replaces `07_THUMBNAIL_V1.md`**
...
## ✅ What the delivery half no longer has to solve

V1 listed getting a thumbnail onto YouTube as part of this file's problem. It is not any more — both delivery faults were fixed in `worker.py`, which is **one file shared by both brands**, so HU inherits them without a second patch (file 09 has the detail).

- `yutu`'s silent refusal of an absolute `--file` path: fixed, `cd` + basename.
- Non-16:9 input: `make_16x9()` centre-crops and scales to 1280×720 before upload.

**Proven on a live QDE run 2026-09-19** — `thumbnail set (1280x720)`, `youtube_thumbnail_set: true`, and the custom thumbnail visibly displaying on `S6JgRfTbdjY`. That is the first time a designed thumbnail has actually reached the channel through this pipeline.

⚠️ `make_16x9()` **centre-crops.** It guarantees the aspect ratio, not the composition — a headline sitting near the top or bottom will be clipped. HU's build script sets the 1280×720 canvas directly, so this should never fire on HU. If it does, treat it as a bug in the build script, not as the safety net doing its job.
...
## ⚠️ Shorts do not display a custom thumbnail

A video YouTube files as a Short shows a frame from the video in the Shorts feed, not the uploaded thumbnail. Building and setting one is still correct — it appears on the watch page and in search — but do not treat a Short as proof the thumbnail pipeline works, and do not spend build time on a cover nobody will see in the shelf. Whether a given file lands as a Short is decided in file 02, and the boundary is finer than it looks.

## Hard requirements carried from the QDE fixes
...
⛔ **The upload step must prove it.** `thumbnails.maxres` present at 1280×720 on `videos.list` is the proof (file 12). `rc == 0` is not — see file 09.
