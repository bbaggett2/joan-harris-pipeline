...
**Cross-references no longer name an exact version.** V11 still pointed at
`hu_description_prompt_V2.md` by name, which would go stale the moment that prompt bumps.
That is the only change — every rule, phase, setting and open question is identical to V11.

<details>
<summary><strong>Earlier changes</strong> (kept as history)</summary>

**V11** — removed the resolved open flag about the duplicate `joan-supervising-agent/README.md`,
which had been deleted and replaced by an unnumbered folder README.

**V10** —

1. **YouTube upload is the `yutu` CLI**, from a local file path. V9 said YouTube Studio in
   Chrome and explicitly banned `yutu`, which contradicted the pre-flight checklist Gate 4.
   Bart settled it 2026-09-11.
