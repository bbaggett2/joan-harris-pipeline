# HU VIDEO PUBLISHING PIPELINE — V11

**Brand:** Handwriting University (HU) ONLY
**Date:** 2026-09-12
**Location:** `joan-supervising-agent/` — cross-agent document, not Betty's
**Status:** AUTHORITATIVE for HU — supersedes V10

> **Versioning:** the version lives in the filename, the highest number is current, and
> every change bumps it. `..._V11.md` beats `..._V10.md`, always. The header version above
> must match the filename. A new revision becomes `..._V12.md`, and this file is deleted in
...
1. **The resolved README flag is gone.** V10's OPEN FLAGS #5 said the duplicate
   `joan-supervising-agent/README.md` was still live and should be deleted. It has been
   deleted, and an unnumbered folder README replaced it. The flag pointed at finished work
   and at `README_V2.md`, which no longer exists.
2. **Remaining flags renumbered.** No flag was dropped except the resolved one.

**No procedural change.** Every rule, phase, setting and open question below is identical
to V10. The number moved because every change bumps it — that is the point of the rule, not
an exception to it.
...
1. **YouTube upload is the `yutu` CLI**, from a local file path. V9 said YouTube Studio in
   ...
   `Joan_PreFlight_Pipeline_Checklist` Gate 4. Bart settled it 2026-09-11.
2. **Metricool posts are created as DRAFTS.** Agents never schedule a post. V9's
   unconditional `autoPublish: true, draft: false` at creation was replaced by a two-stage
   rule.
3. **The Drive-URL ban was narrowed.** V9 said Google Drive URLs always fail as Metricool
   media. They fail when the file is owner-only to an editor's Google account.
4. **Four approval gates became one review.**
5. **"Bart" was replaced by the REVIEWER role.**
6. **Facebook page ID verification merged in** from the duplicate README copy.
7. **Open questions marked in place** rather than assumed.
...
5. **CTA variants** — two approved CTAs in circulation. Confirm which is canonical.
6. **`BART_BAGGETT_VOICE.MD` is committed three times** — `brands/`, `betty/`, `peggy/`,
   all identical. Keep `brands/`. Delete the other two.
7. **Bart Show / BAB** — `brands/bartallanbaggett-brand.md` exists, but no description
