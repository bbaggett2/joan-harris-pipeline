# QDE VIDEO PUBLISHING PIPELINE — V10
...
1. **YouTube upload is the `yutu` CLI**, from a local file path. V9 said YouTube Studio in
   Chrome and explicitly banned `yutu`. Bart settled it 2026-09-11. Phase 7 caption upload
   is a separate step and is **not** `yutu`.
2. **Metricool posts are created as DRAFTS.** Agents never schedule. The reviewer
   schedules, and that act is the approval.
3. **The Drive-URL ban is narrowed** to the case it actually describes — a file owner-only
   to an editor's Google account. The FTP bridge is a fallback, not the procedure.
4. **Four sign-offs become one review**, done where the work already sits.
5. **"Bart" is replaced by the REVIEWER role.**

QDE-specific:

6. **Facebook page ID corrected.** V9 recorded `handwritingexpertsinc` — that is the
   handle. The page ID is `120481384817826`, read from the live Metricool account
   2026-09-11.
7. **Network set filled in.** V9 left it `[TO VERIFY]`. Six networks are connected. The
   *scope* question — whether LinkedIn and GBP receive video posts — is flagged, not assumed.
8. **YouTube channel filled in at Phase 6.** V9 recorded it in the config block but left
   Phase 6 step 2 as `[TO VERIFY]`.
9. **Gate 7 legal accuracy is preserved and strengthened.** It is now an explicit,
   non-skippable element of the single review. See THE REVIEW MODEL.
...
Identical to the HU pipeline. See `HU_Video_Publishing_Pipeline_V10.md` Phase 1 for the
