# QDE BLOG PRODUCTION SOP v6 — celebrity & historical case articles on bartbaggett.com/blog

Owner: Cameron (blog production, WordPress and technical fixes). Created 2026-09-23. v2 (2026-09-23): blog notices moved to #qde-blog-post-project; Kristine's role is proofreading; n8n publish rule added. v3 (2026-09-23): errors and queue warnings go to #n8n-alerts (Bart); Kristine's channel only gets drafts to proofread. v4 (2026-09-23): n8n workflows renamed from "Betty" (social media) to "Cameron" (blog production). v5 (2026-09-23): schema is no longer stored in the post; the WPCode snippet "QDE blog schema (AEO)" builds it from the live post on every page load. v6 (2026-09-23): schema snippet v2 (clean apostrophes in headline); after pasting a snippet, make sure its toggle says Active and clear WP Super Cache.
...
| Schema (BlogPosting + speakable + FAQPage) | Cameron | cameron-tech-support/QDE_Blog_Schema_Snippet_v2.php (pasted into WPCode) | No (runs inside WordPress) |

When any of these files gets a new version number, the matching URL in the n8n workflow's Config node must be changed the same day (prompt_url, voice_url, closing_url, image_prompts_url). A new schema snippet version must be pasted into WPCode the same day.
...
5. **Publishing:** nothing is ever published automatically. Kristine proofreads (image checklist in Salvatore's SOP); fact questions from the TO VERIFY list go to Bart; a person publishes by hand. The schema appears by itself the moment the post is published.
...
- **Schema (AEO):** WPCode PHP snippet "QDE blog schema (AEO)", Run Everywhere, source cameron-tech-support/QDE_Blog_Schema_Snippet_v2.php. On every published post that has the public FAQ (<!-- qde-faq-start -->) it prints BlogPosting (speakable -> #speakable-summary) + FAQPage in the page head, read from the live title, Yoast meta description, featured image, dates and FAQ. Kristine's edits show up automatically; drafts get no schema until published. It also hides old schema blocks stored inside earlier posts. Do not deactivate it. After pasting a new version: toggle Active, save, then WP Super Cache > Delete Cache. If the site firewall (Wordfence) blocks the save, the admin marks it a false positive and saves again. Check a published post with Google's Rich Results Test.
