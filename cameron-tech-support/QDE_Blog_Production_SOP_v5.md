# QDE BLOG PRODUCTION SOP v5 — celebrity & historical case articles on bartbaggett.com/blog
...
| Schema (BlogPosting + speakable + FAQPage) | Cameron | cameron-tech-support/QDE_Blog_Schema_Snippet_v1.php (pasted into WPCode) | No (runs inside WordPress) |
...
- **Schema (AEO):** WPCode PHP snippet "QDE blog schema (AEO)", Run Everywhere, source cameron-tech-support/QDE_Blog_Schema_Snippet_v1.php. On every published post that has the public FAQ (<!-- qde-faq-start -->) it prints BlogPosting (speakable -> #speakable-summary) + FAQPage in the page head, read from the live title, Yoast meta description, featured image, dates and FAQ. Kristine's edits show up automatically; drafts get no schema until published. It also hides old schema blocks stored inside earlier posts. Do not deactivate it. Check a published post with Google's Rich Results Test.
