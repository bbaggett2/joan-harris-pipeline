# Handwriting University — Brand Kit

**Brand ID:** `handwriting-university` / `HU`
**Version:** 2026.1
**Owner:** Bart Baggett, Founder
**File purpose:** Single source of truth for all HU artwork, graphics, thumbnails, articles, captions, and AI generation. If you are producing anything under the HU brand, this file governs it.

---

## 0. How to use this file (agent instructions)

1. **Confirm the lane first.** Read §1.2. If the content is forensic/legal or personal-media, this is the wrong brand file — stop and flag it.
2. **Read §2 (Voice) before writing any words.** The banned-terms rule in §2.3 is absolute and is the most common failure point.
3. **Read §4–§8 before generating any image.** Copy the master prompts in §8 verbatim and swap only the bracketed variables.
4. **Run §11 (QA Checklist) before delivering anything.**
5. **Never invent a credential, statistic, course name, price, or URL.** If it isn't in this file or supplied in the task, mark it `[TO VERIFY]` and ask.

Items marked **`[TO VERIFY]`** in this document are placeholders that have not been confirmed by Bart. Do not publish them as fact.

---

## 1. Brand Identity

### 1.1 What HU is

Handwriting University is the **education and handwriting-psychology** brand. It teaches people to read personality, character, and emotional patterns from handwriting — and to change traits by changing strokes.

The visual tone balances **academic prestige and scientific authority** with **cinematic, high-contrast engagement**. Sophisticated, collegiate but accessible, precise, and intriguing. It should feel like a serious university department that also knows how to hold attention on a phone screen.

**Audience:** curious general public, students of personality and psychology, coaches, HR and hiring professionals, writers, and self-improvement seekers.

**Primary domain:** HandwritingUniversity.com

### 1.2 Lane separation — critical

Bart operates three distinct brands. They must never bleed into each other. Check this table before producing anything.

| Signal in the content | Correct brand | Action if not HU |
| :--- | :--- | :--- |
| Personality, character traits, relationships, self-improvement, teaching handwriting analysis, student courses | **HU (this file)** | Proceed |
| Forgery, signature authentication, questioned documents, will contests, expert witness, court testimony, attorney-facing | **QDE / Handwriting Experts Inc.** | STOP. Use the QDE brand file. Domain is HandwritingExpertUSA.com, phone 1-800-980-9030. |
| Culture, freedom, current events, podcast, personal opinion | **The Bart Show / Bart Allan Baggett** | STOP. Use the Bart Allan Baggett brand file. |

Never place QDE credentials (court testimony counts, judicial acceptance rate, expert-witness claims) inside HU copy. Never place HU personality-analysis claims inside QDE copy.

### 1.3 Core imagery motifs

- Classic feather quills and fountain pens with silver nibs
- Aged antique parchment (flat fill, not heavy grain)
- Deep ink strokes, magnified
- Handwriting under magnification — loupe, magnifying glass
- Bright teacher-pen red corrections and circles
- Dark academic study interiors at dusk

---

## 2. Voice & Language

### 2.1 Tone

Confident, warm, plainspoken, and curiosity-driven. Bart teaches — he does not lecture. Short declarative sentences. Concrete examples over abstraction. A reader should feel like they just learned something they can test on their own handwriting in thirty seconds.

- Write in **first person** when the speaker is Bart.
- Address the reader as **"you."**
- Favor the specific over the general: "the lower loop on your *y*" beats "certain letter formations."
- It is fine to be provocative in a hook. It is not fine to be sensational in a claim.

### 2.2 Structural defaults

- **Articles:** Open with a hook or a question the reader is already asking. Deliver the payoff in the first 100 words — never bury it. Use descriptive subheads. Close with a single clear CTA.
- **Video descriptions / captions:** Lead with the strongest line. Body content must be built from the **actual transcript**, never inferred from the title. If no transcript is available, stop and request it.
- **Short-form captions (TikTok / IG Reels):** Under 1,000 characters. Use the video title as the hook when it is stronger than a rewrite. Always supply on-screen hook options plus a second caption variation, without being asked.

### 2.3 Banned and restricted terms — absolute

| Term | Rule |
| :--- | :--- |
| **"graphology" / "graphologist"** | **BANNED from all HU body copy**, titles, descriptions, on-screen text, and article prose. Permitted **only** as a hashtag (`#graphology`). Use "handwriting analysis" and "handwriting analyst" instead. |
| **"win your case" / outcome promises** | Banned. This is QDE-adjacent language and HU makes no legal claims at all. |
| **HandwritingExpert.com** | Never write this domain in any HU or QDE copy. It is not owned. |
| Unverified credentials, stats, or testimonials | Banned. Verified only. Mark anything uncertain `[TO VERIFY]`. |

### 2.4 Claims discipline

Handwriting analysis is presented as a **tool for insight**, not as a diagnostic or predictive instrument. Do not claim it detects disease, predicts crime, or guarantees outcomes. Do not attribute analysis to a named living person's handwriting without a documented public sample and Bart's approval.

---

## 3. Color Palette

| Role | Name | Hex | Usage |
| :--- | :--- | :--- | :--- |
| Primary base | **HU Navy** | `#1D3557` | Main brand color, logo text, parchment-block headlines, primary accents |
| Accent / metal | **Silver Quill** | `#C0C0C0` (or metallic gradient) | Logo quill, metallic text edges, subtle highlights |
| Parchment accent | **Parchment Cream** | `#F5F5DC` | Torn parchment blocks, highlight banners, text backdrops — **flat fill, no heavy grain** |
| Contrast / alert | **Teacher Red** | `#D62828` | Magnification circles, callouts, highlight strokes, UI alerts |
| Raw pen red | **Pen Red** | `#FF0000` | Simulated red-pen annotation strokes only — the "wet ink" look *(recommendation, not yet ratified by Bart)* |
| Text primary | **Pure White** | `#FFFFFF` | Thumbnail headlines, overlay text on dark |
| Background | **Cinematic Dusk** | `#0B1320` → `#101820` | Darkened study, dusk window light, deep photographic depth |

**Hard rules:**
- **Never use brown as a primary or dominant background color.**
- Parchment is an *accent*, never a full-canvas background.
- Maintain minimum 4.5:1 text contrast on all web body copy.

---

## 4. Typography

| Level | Font | Rules |
| :--- | :--- | :--- |
| **Display / headline** | Heavy collegiate block-serif, or `Impact` | ALL CAPS. Hook lines, big numbers ("10,000 TIMES"), thumbnail headlines. White with metallic-silver edge on dark backgrounds. |
| **Parchment block text** | `Georgia Bold` or classic serif | Rendered in HU Navy `#1D3557`. Used for text sitting on parchment. |
| **Body / web prose** | `Inter`, `Roboto`, or `Helvetica Neue` | Clean sans-serif, high legibility. Web pages, descriptions, articles. |

Do not mix more than two typefaces in a single graphic. Never set body copy in Impact.

---

## 5. Logo

The HU mark is a navy **"HU"** paired with a **silver quill** accent.

**Placement rules:**
- YouTube 16:9 → bottom-left or top-right, small. Never dominant. **Keep bottom-right clear** for the YouTube timestamp overlay.
- Vertical 9:16 → bottom-left, approximately **148px wide** on a 1080×1920 canvas.
- Never stretch, recolor, rotate, or place on a low-contrast background.

**Asset file:** `HU logo new 2026 Watermark.png`, used **unaltered** — do not re-export, recolor, or "clean up." It is a padded PNG, so measure placement against the **visible mark**, not the file box. It lives in `salvatore-art-department/`; see that folder's process file §7 for the two known asset quirks Bart has already reviewed and accepted.

**Other variants** (light/dark/mono, web favicon, print): `[TO VERIFY]` — none supplied.

---

## 6. YouTube Thumbnail SOP (16:9)

Canvas 1280×720. Maximum **three** visual focus elements.

1. **Presenter — right ~40%.** Headshot, Waist-up, **entire head fully visible, never cropped.** The emotion and body language will come from the content of the transcript.  Host always looking directly into camera.
2. **Visual trait focus — left ~40%.** One large, clearly legible magnified handwritten cursive letter or word, circled or highlighted in bright teacher-red pen. The source of this image will always come from published material from Bart Baggett's Libary of approved trait images. https://drive.google.com/drive/folders/1_ZzlXw1Zr2hoT-2aW8NK1cbWq8L2j-gH
3. **Headline text — upper left.** Heavy collegiate block-serif, ALL CAPS, white with metallic-silver edge. The single key word or phrase sits inside a slightly tilted antique-parchment rectangle with navy text. The parchment should look like paper. Sometimes the edges are torn, but often they appear cut with scissors.  That depends on the size of the space required.
4. **Logo.** Small, bottom-left or top-right. Bottom-right stays clear.
5. **Background.** Dark cinematic study at dusk, real photographic depth, non-distracting. The background is generated at a different step by Gemini. Building a background as a different solo step using Gemini produces better results and less drift than asking gemini to assemble the thumbnail with background and words in one prompt. 

**Text limit:** 3–5 words maximum. It must be readable at 168×94px.

---

## 7. Vertical Covers (9:16 — Instagram Reels, TikTok, Shorts)

**This spec is not in this file, by design.** The 9:16 cover method is owned by the art department and documented in full there:

```
salvatore-art-department/HU_Thumbnail_Creation_Process.md
```

Read that file before producing any 9:16 cover. Do not work from a summary, from memory, or from an older version of this brand kit — the earlier "top 40% full-bleed parchment band" spec has been **retired** and any output built to it is wrong.

Two things you need to know before you go there:

1. **The method is a deterministic Python build (`build_hu_ig_thumbnail.py`), not a prompt.** Do not hand it to an image model. If the script is unavailable, stop and say so rather than substituting a generated approximation.
2. **Bart approves. Nothing else does.**

---

## 8. Master AI Image Prompts

Copy these verbatim. Replace only `[BRACKETED]` variables. Do not add brown, do not add extra subjects, do not add text beyond what is specified.

### 8.1 YouTube 16:9 thumbnail

```text
A professional 16:9 YouTube thumbnail for a psychology and handwriting analysis channel.
SUBJECT: A headshot of the male host on the right side (40% width), shoulders-up, head fully
visible, confident and engaged expression looking directly at the camera.
BACKGROUND: Dark cinematic study at dusk with soft lighting, high contrast, genuine
photographic depth, non-distracting.
VISUAL ELEMENT (left side): A magnified, highly legible sample of handwritten cursive text on
high-quality parchment, with [TRAIT] circled in bright red teacher-pen.
TEXT OVERLAY: Bold collegiate block-serif headline in pure white with a subtle metallic silver
edge on the upper-left, reading "[HEADLINE]". The key word "[KEYWORD]" placed inside a slightly
tilted antique-parchment block in navy blue lettering.
BRANDING: Small Handwriting University logo in the corner.
Clean composition, maximum 3 visual focus elements, bottom-right clear. No brown backgrounds.
```

### 8.2 Vertical 9:16 Reel / Short cover — **REMOVED**

There is no AI prompt for 9:16 covers. The earlier prompt described the retired full-bleed parchment band and would produce wrong output. See §7 — the method is a script in the art department folder.

### 8.3 Website header banner

```text
A high-end, cinematic website banner for Handwriting University (16:9 aspect ratio). Dark navy
(#1D3557) ambient study background with soft warm accent lighting on a dark wooden desk. On the
desk lies a pristine fountain pen with a silver nib, an open vintage journal with crisp cursive
handwriting, and a magnifying glass focusing on an intricate ink stroke. Professional, academic,
authoritative, sleek, modern lighting with high contrast. No text.
```

### 8.4 Article / blog hero image — `[UNRATIFIED DRAFT]`

> **Not Bart's.** 8.1 and 8.3 above are Bart's own approved prompts. This one was drafted by an agent to fill a gap and has **not** been reviewed or approved. Use it as a starting point only, and say so when you do.

```text
A cinematic editorial header image for a handwriting psychology article, 16:9. Close macro
photograph of [SUBJECT: e.g. a fountain pen mid-stroke on cream paper / a magnifying glass over
cursive script]. Dark cinematic dusk background (#0B1320), navy and silver tones, soft directional
warm key light, shallow depth of field, genuine photographic realism. Academic and authoritative.
No text, no people, no brown dominant tones.
```

---

## 9. Article & Blog Standards

> **Structure and length below are `[UNRATIFIED DRAFT]`** — drafted by an agent, not supplied by Bart. The voice, attribution, and CTA lines are enforceable; treat the rest as a placeholder until Bart rules on it.

- **Length:** `[TO VERIFY]` — no house word count confirmed. Default to 900–1,400 words for educational posts unless told otherwise.
- **Structure:** hook → payoff → 3–5 teaching sections with descriptive subheads → practical "try this on your own handwriting" step → single CTA.
- **Imagery:** one hero image (§8.4) plus at least one magnified handwriting sample with a red-pen callout.
- **Voice:** first person, second-person address, no graphology, no outcome promises.
- **Attribution:** Bart's own books and published work may be cited. If quoting Bart or any book, the quote must be verified against source text — never reconstructed from memory. Flag unverified quotes explicitly.
- **CTA:** point to HandwritingUniversity.com. Never to QDE assets.

---

## 10. Hashtags

Use 5–12 relevant tags. `#graphology` is permitted **here only** — never in body copy.

Core set: `#handwritinganalysis` `#handwriting` `#graphology` `#personality` `#psychology` `#handwritinguniversity` `#bartbaggett` `#selfimprovement`

Do not use QDE-specific tags (`#HandwritingExpert`, forensic/legal tags) on HU content.

---

## 11. QA Checklist — run before delivering

- [ ] Correct brand lane confirmed (§1.2) — this is not QDE or Bart Show content
- [ ] The words "graphology" / "graphologist" appear nowhere in body copy, titles, or on-screen text
- [ ] No outcome promises, no unverified stats, no unverified credentials
- [ ] Colors match §3 exactly; no brown dominant background
- [ ] Parchment is flat fill, not heavy grain
- [ ] Presenter's head fully visible, uncropped, unobstructed
- [ ] 16:9 thumbnail readable at 168×94px; ≤5 words of text; bottom-right clear
- [ ] Captions built from actual transcript, not from the title
- [ ] CTA points to HandwritingUniversity.com only
- [ ] Every `[TO VERIFY]` and `[UNRATIFIED DRAFT]` item resolved or explicitly flagged to Bart

**For 9:16 covers, this checklist does not apply.** Use the one in `salvatore-art-department/HU_Thumbnail_Creation_Process.md` §8.

---

## 12. Open items requiring Bart's confirmation

These are unresolved. Do not guess.

1. Logo asset file paths and approved variants (light/dark/mono).
2. Official tagline or positioning line for HU.
3. Ratification of the `#D62828` vs `#FF0000` split proposed in §3 — **agent's proposal, not Bart's.**
4. Licensed display typeface — "heavy collegiate block-serif" is a description, not a named font.
5. House article word count and standard CTA wording.
6. Approved course/product names and pricing for CTA copy.
7. Approved presenter photo library and any restrictions on which headshots may be used.
8. Whether the §6 16:9 YouTube layout still stands. It predates the 9:16 torn-parchment method and was **not** superseded by it — that method declares itself 9:16 only — but the two have never been reconciled.
9. Ratification of §8.4 and the §9 article structure, both currently `[UNRATIFIED DRAFT]`.
10. Liar Liar reference: is `ig_v2.jpg` or `ig_v3` canonical? See the art department file §10.2.
